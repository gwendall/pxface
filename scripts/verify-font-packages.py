#!/usr/bin/env python3
"""Verify PXFACE font metadata, artifacts, checksums, and exact archive trees."""
...
EXPECTED_ARCHIVES = {
    f"PXFACE3x5-TTF-v{VERSION}.zip": {
        "OFL.txt", "README.txt", f"{FILE_STEM}.ttf",
    },
    f"PXFACE3x5-OTF-v{VERSION}.zip": {
        "OFL.txt", "README.txt", f"{FILE_STEM}.otf",
    },
    f"PXFACE3x5-Web-v{VERSION}.zip": {
        "OFL.txt", "README.txt", f"{FILE_STEM}.woff", f"{FILE_STEM}.woff2", "pxface-3x5.css",
    },
    f"PXFACE3x5-v{VERSION}.zip": {
        ...
        f"web/{FILE_STEM}.woff", f"web/{FILE_STEM}.woff2", "web/pxface-3x5.css",
    },
}
...
def verify_font_metadata(relative_path: str):
    ...
    assert names.getDebugName(1) == "PXFACE 3x5", f"Wrong family name: {relative_path}"
...
def main():
    ...
    assert manifest["reservedFontName"] == "PXFACE"
    ...
    with zipfile.ZipFile(FONTS / f"PXFACE3x5-Web-v{VERSION}.zip") as archive:
        css_name = f"PXFACE3x5-Web-v{VERSION}/pxface-3x5.css"
        css = archive.read(css_name).decode("utf-8")
        assert 'url("./PXFACE3x5-Regular.woff2")' in css, "Web kit CSS must use portable relative URLs"
        assert f"v{VERSION}" not in css, "Web kit CSS must not depend on pxface.com paths"
